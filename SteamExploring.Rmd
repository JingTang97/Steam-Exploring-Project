...
# Benford Analysis
In this part, this project used sales data of games on Steam over past half of year(from Jun 2018 to Dec 2018) for verifing. This data including sales records of 9623 games in total.
```{r}
# Choose sales volume data of each game
bfd.cp <- benford(Steam_sales$`Approximately Sales Volume`)
bfd.cp
...
```
\par
Accoring to the plot, we can get the conclusion that there are some inconsistencies on lake data. The majority data obey the distribution of benford, but it shows the 10 largest discrepancies. From the table of duplicates data, the most probably duplicated number is 200, we can see that over 470 games sold 200 times over the past half of year which is extremly weird. Excepted that, 300 and 400 are also among the top 10 duplicates data. Insteresting.
\par
